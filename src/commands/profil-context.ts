import { ApplicationCommandType, ContextMenuCommandBuilder, GuildMember, PermissionFlagsBits, UserContextMenuCommandInteraction, User } from 'discord.js';
...
async function enrichUser(user: User): Promise<User> {
	try { return await user.fetch(true); } catch { return user; }
}
...
async function execute(interaction: UserContextMenuCommandInteraction): Promise<void> {
	const targetUser = interaction.targetUser as User;
	const inGuild = Boolean(interaction.guild);
	const fetchedUser = await enrichUser(targetUser);
	const member = await resolveMemberSafe(interaction, targetUser);
